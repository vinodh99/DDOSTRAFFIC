...
NS_LOG_COMPONENT_DEFINE ("HttpServertrafficApplication");

TypeId
HttpServertraffic::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::HttpServertraffic")
    .SetParent<Application> ()
    .AddConstructor<HttpServertraffic> ()
    .AddAttribute ("Port",
                   "Port on which we listen for incoming packets.",
                   UintegerValue (80),
                   MakeUintegerAccessor (&HttpServertraffic::m_port),
                   MakeUintegerChecker<uint16_t> ())
  ;
  return tid;
}

HttpServertraffic::HttpServertraffic ()
{
...
void
HttpServertraffic::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  Application::DoDispose ();
}

void HttpServertraffic::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if (!m_socket)
    {
      ...
      if (m_socket->GetSocketType () != Socket::NS3_SOCK_STREAM &&
        ...
        {
          NS_FATAL_ERROR ("Using HttpServertraffic with an incompatible socket type. "
                          "HttpServertraffic requires SOCK_STREAM or SOCK_SEQPACKET. "
      ...
      m_socket->Bind (local);
      m_socket->Listen ();
      m_socket->SetAcceptCallback (MakeCallback (&HttpServertraffic::HandleRequest, this),
                                   MakeCallback (&HttpServertraffic::HandleAccept, this));
    }
}

void HttpServertraffic::StopApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_socket != 0)
    ...
    {
      NS_LOG_WARN ("HttpServertrafficApplication found null socket to close in StopApplication");
    }
}


bool
HttpServertraffic::HandleRequest (Ptr<Socket> s, const Address& address)
{
  NS_LOG_FUNCTION (this << s << address);
  NS_LOG_DEBUG ("HttpServertraffic >> Request for connection from " << InetSocketAddress::ConvertFrom (address).GetIpv4 () << " received.");
  return true;
}


void
HttpServertraffic::HandleAccept (Ptr<Socket> s, const Address& address)
{
  NS_LOG_FUNCTION (this << s << address);

  NS_LOG_DEBUG ("HttpServertraffic >> Connection with Client (" <<  InetSocketAddress::ConvertFrom (address).GetIpv4 () << ") successfully established!");
  s->SetRecvCallback (MakeCallback (&HttpServertraffic::HandleReceive, this));
}


void
HttpServertraffic::HandleReceive (Ptr<Socket> s)
{
  ...
  NS_LOG_INFO ("HttpServertraffic >> Client requesting a " + url);
  ...
  if (url == "main/object")
    {
      ...
      NS_LOG_INFO ("HttpServertraffic >> Sending response to client. Main Object Size ("
  ...
  else
    {
      ...
      NS_LOG_INFO ("HttpServertraffic >> Sending response to client. Inline Objectsize ("
